  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (found_indices) {
      ...
      if (final_indices) {
        ...
        return OneHotMatch{final_indices, weights, i == 0, contract_dim};
      }
    }
  }
  return std::nullopt;
...
// 1. Reshapes the Indices tensor to add a trailing dimension of size 1.
// 2. Configures a Gather instruction to slice the Weights tensor along the
//    contracting dimension using the reshaped Indices.
// 3. Replaces the original Dot instruction with the new Gather instruction.
absl::Status RewriteOneHotDotToGather(HloComputation* computation,
  ...
  // 1. Identify bounds and create constants for clamping.
  ...
  // 2. Robustness - Clamp indices to valid memory range [0, depth-1].
  ...
  // 3. Robustness - Create in-bounds mask: (indices >= 0) && (indices < depth).
  ...
  // 4. Robustness - Broadcast mask to match the Dot output shape.
  ...
  // 5. Robustness - Select between Gather result and Zeros.
