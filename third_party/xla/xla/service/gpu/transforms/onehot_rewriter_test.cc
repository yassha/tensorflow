...
%_one_hot_1296.528 (Arg_0.320: s32[2,2,2]) -> pred[2,2,2,8] {
  %Arg_0.320 = s32[2,2,2]{2,1,0} parameter(0)
  %broadcast_in_dim.2564 = s32[2,2,2,1]{3,2,1,0} reshape(%Arg_0.320)
  %eq.327 = s32[2,2,2,1]{3,2,1,0} broadcast(%broadcast_in_dim.2564), dimensions={0,1,2,3}
  %eq.328 = s32[2,2,2]{2,1,0} reshape(%eq.327)
  %eq.329 = s32[2,2,2,8]{3,2,1,0} broadcast(%eq.328), dimensions={0,1,2}
  %iota.205 = s32[8]{0} iota(), iota_dimension=0
  %iota.206 = s32[1,1,1,8]{3,2,1,0} reshape(%iota.205)
  %eq.330 = s32[1,1,1,8]{3,2,1,0} broadcast(%iota.206), dimensions={0,1,2,3}
  %eq.331 = s32[8]{0} reshape(%eq.330)
  %eq.332 = s32[2,2,2,8]{3,2,1,0} broadcast(%eq.331), dimensions={3}
  ROOT %eq.333 = pred[2,2,2,8]{3,2,1,0} compare(%eq.329, %eq.332), direction=EQ
}

ENTRY %main {
  %jit__where_.369 = s32[2,2,2]{2,1,0} parameter(0)
  %jit__one_hot_.3 = pred[2,2,2,8]{3,2,1,0} call(%jit__where_.369), to_apply=%_one_hot_1296.528
  %dot_general.206 = bf16[2,2,2,8]{3,2,1,0} convert(%jit__one_hot_.3)
  %Arg_9.23 = bf16[8,2,2,2]{3,2,1,0} parameter(2)
  %reshape.520 = bf16[8,2,4]{2,1,0} reshape(%Arg_9.23)
  %dot_general.207 = bf16[2,2,2,2,4]{4,3,2,1,0} dot(%dot_general.206, %reshape.520), lhs_contracting_dims={3}, rhs_contracting_dims={0}
  %dot_general.201 = bf16[2,2,2,8]{3,2,1,0} convert(%jit__one_hot_.3)
  %Arg_8.23 = bf16[8,2,2,2]{3,2,1,0} parameter(1)
  %reshape.519 = bf16[8,2,4]{2,1,0} reshape(%Arg_8.23)
  %dot_general.202 = bf16[2,2,2,2,4]{4,3,2,1,0} dot(%dot_general.201, %reshape.519), lhs_contracting_dims={3}, rhs_contracting_dims={0}
...
ENTRY main {
  ...
  // Weights: [2, 4, 8]
  // OneHot: [8, 2, 2, 2] (Assuming Iota is dim 0)
...
ENTRY Main {
  ...
  // Trace through Reshapes and Converts
...
ENTRY Main {
  ...
  // Merging the one-hot dimension into a larger flattened space
...
ENTRY Main {
  ...
  one_hot = f32[10] convert(compare)
  weights = f32[10, 128] parameter(1)
  // Dot output is [128]. If the rewriter uses s32[2, 5] as indices,
  // the resulting Gather would have shape [2, 5, 128], causing a mismatch.
  ROOT dot = f32[128] dot(one_hot, weights), 
             lhs_contracting_dims={0}, rhs_contracting_dims={0}
}
)";
